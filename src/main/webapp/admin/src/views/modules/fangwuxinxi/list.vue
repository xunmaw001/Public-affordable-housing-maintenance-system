<template>
  <div class="main-content">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            ...
            <el-button
              v-if="isAuth('fangwuxinxi','导入') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="success"
              icon="el-icon-plus"
              @click="importHandler()"
            >{{ contents.btnAdAllFont == 1?'导入':'' }}</el-button>
            <el-button
              v-if="isAuth('fangwuxinxi','导入') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="success"
              @click="importHandler()"
            >{{ contents.btnAdAllFont == 1?'导入':'' }}<i class="el-icon-plus el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('fangwuxinxi','导入') && contents.btnAdAllIcon == 0"
              type="success"
              @click="importHandler()"
            >{{ contents.btnAdAllFont == 1?'导入':'' }}</el-button>

	    <download-excel v-if="isAuth('fangwuxinxi','导出')" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "房屋信息.xls">
		      <!-- 导出excel -->
            	<el-button
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('fangwuxinxi','房屋维护') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="fangwuweihuCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'房屋维护':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','房屋维护') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="fangwuweihuCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'房屋维护':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('fangwuxinxi','房屋维护') && contents.tableBtnIcon == 0" type="success" size="mini" @click="fangwuweihuCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'房屋维护':'' }}</el-button>
    ...
    <fangwuweihu-cross-add-or-update v-if="fangwuweihuCrossAddOrUpdateFlag" :parent="this" ref="fangwuweihuCrossaddOrUpdate"></fangwuweihu-cross-add-or-update>
    ...
    <el-dialog
      ...
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item class="upload" label="文件" prop="excelFile">
          <excel-file-upload
          tip="点击上传直接导入excel文件"
          action="fangwuxinxi/importExcel"
          :limit="1"
          ></excel-file-upload>
        </el-form-item>
      </el-form>
...
import fangwuweihuCrossAddOrUpdate from "../fangwuweihu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      fangwuweihuCrossAddOrUpdateFlag: false,
...
//导出excel
  ...
  methods: {
    ...
    fangwuweihuCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.fangwuweihuCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.fangwuweihuCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.fangwuweihuCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
