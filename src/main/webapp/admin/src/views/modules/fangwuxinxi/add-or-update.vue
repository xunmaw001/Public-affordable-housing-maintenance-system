<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="业主号" prop="yezhuhao">
          <el-select :disabled="ro.yezhuhao" @change="yezhuhaoChange" v-model="ruleForm.yezhuhao" placeholder="请选择业主号">
            <el-option
                v-for="(item,index) in yezhuhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.yezhuhao" label="业主号" prop="yezhuhao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/yezhu/yezhuhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.yezhuhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    yezhuhaoChange () {
      this.$http({
        url: `follow/yezhu/yezhuhao?columnValue=`+ this.ruleForm.yezhuhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.yezhushouji){
                this.ruleForm.yezhushouji = data.data.yezhushouji
              }
